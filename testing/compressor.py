...
class CompressorError(Exception):
    ...
    def __str__(self):
        return "{conclusion}".format(conclusion=self.conclusion)
...
class Compressor:
    ...
    def execute(self, input_f, output_f, mode, name):
        with temp_filepaths('input') as (temp_input_f,):
            # Copy input file to temporary location
            cwd = os.getcwd()
            os.chdir(self.test_dir)
            shutil.copy2(input_f, temp_input_f)
            os.chdir(cwd)

            args = [sys.executable, self.exe_path,
                    '--input', temp_input_f,
                    '--output', output_f,
                    '--mode', mode,
                    '--method', self.method]
    ...
    def run_test(self):
        with temp_filepaths('compressed', 'decompressed') as (compressed, decompressed):
            ...
            try:
                ...
                if os.environ.get('CHECKER'): raise e
